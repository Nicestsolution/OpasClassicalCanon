...
// Scroll & state
let savedScroll = 0;
let lastSongElId = null;

// If your playlist scrolls inside a container, use this as scroller.
// If the page scrolls as a whole, keep scroller = window.
const scroller = document.getElementById('playlist') || window;
...
/* ========================
   Rendering functies
======================== */
// ... hier blijven je bestaande functies zoals
// formatLifeSpan(), createSongDetailBlock(), renderPlaylist(),
// renderComposers(), renderComposerTimeline(), showComposerDetails()
// ongewijzigd, maar waar je nu toggle logica hebt kun je
// toggleVisibility(showEl, hideEl) gebruiken.
...
function formatLifeSpan(composer) {
  ...
  if (birth || death || placeOfBirth || placeOfDeath) {
    if (!death) {
      return `${placeOfBirth}: ${birth} – heden`;
    }
    return `${placeOfBirth}: ${birth} – ${placeOfDeath}: ${death}`;
  }
  return "";
}

// createSongDetailBlock
...
function renderPlaylist(selectedPlaylist) {
	...
	if (spotifyURL) {
	  ...
	  spotifyLink.innerHTML = `
		...
		${playlistName} 🎧 Luister deze playlist op Spotify
		</a>
	  `;
...
function showComposerDetails(id, fromView) {
  ...
  closeBtn.addEventListener('click', () => {
    if (fromView === 'timeline' || fromView === 'composers') {
      updateView(); // terug naar rechter selector
    } else {
      backToPlaylist(); // terug naar playlist + scroll herstel
    }
  });
